...
import { formatCarBrand } from "./utils";
...
export default (initLoading = true) => {
  ...
  const fetch = async (userId, country) => {
    ...
    try{
      ...
      const formattedResponse = formatCarBrand(response.data.body);
      const formatSelect = selectFormat(response.data.body);
      setSelectData(formatSelect);
