...
import { put } from "../GarageSpecialties/api";
import { get, getById, post, del } from "./api";
import { formatCarOrigins } from "./utils";
...
export default (initLoading = true) => {
  ...
  const fetch = async (userId, country) => {
    ...
    try{      
      ...
      const formattedResponse = formatCarOrigins(response.data.body);
      const selectResponse = selectFormat(response.data.body);
      setData(formattedResponse);
      setSelectData(selectResponse);
    } catch(e){
      console.log(e)
    } finally{
      setIsFetching(false);
